...
$GUI 		= GUICreate("iPhatLoc", 130, 250, -1, -1, BitXOR($GUI_SS_DEFAULT_GUI, $WS_MINIMIZEBOX))
$start 		= GUICtrlCreateButton("START", 4, 4, 120, 41, 0x0001)
Global $tolerance = GUICtrlCreateInput("150", 4, 48, 120, 20, $ES_NUMBER)
...
Func _ShowImg($array, $time)
	...
	$w = $total * 102
	$default = $w
	$GUIRESULT = GUICreate("KẾT QUẢ: " & $total & " CON BÀI, THỜI GIAN: " & Round($time/1000) & " GIÂY.", $w + 10, 152, 1, @DesktopHeight - $height, BitXOR($GUI_SS_DEFAULT_GUI, $WS_MINIMIZEBOX))
	$w = 10
	For $i = 0 To $total - 1
...
Func _GetKey()
	...
	Local $File 		= "server1/f64ccb3c2605785c2b9fe2456a39735b.php?token=" & $Random5 & _Base64Encode($Mac_address) & $Random10 & "&key=" & _RandomAlphaNum(74) & "&game=iphatloc"
	Local $URL 			= $Host & $File
